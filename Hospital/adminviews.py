...
from .models import Specialization,Doctor,Appointment,Page,AddPatient,MedicalHistory,CustomUser,Patient
...
def ViewDoctorDetails(request,id):
    doctorlist1=Doctor.objects.filter(id=id)
...
def Search_Doctor(request):
    if request.method == "GET":
        ...
        if query:
            ...
            searchdoc = Doctor.objects.filter(mobilenumber__icontains=query) | Doctor.objects.filter(admin__first_name__icontains=query) | Doctor.objects.filter(admin__last_name__icontains=query)
...
def Doctor_Between_Date_Report(request):
    ...
    if start_date and end_date:
        ...
        doctor = Doctor.objects.filter(regdate_at__range=(start_date, end_date))
...
def DELETE_REGUSERS(request, id):
    try:
        patreg = get_object_or_404(Patient, id=id)
...
def Reg_User_Appoinments(request,id):
    pat_admin = Patient.objects.get(id=id)
...
def view_pending_doctors(request):
    ...
    return render(request, 'admin/view_pending_doctors.html', context)

def approve_doctor(request, doctor_id):
    doctor = get_object_or_404(Doctor, id=doctor_id)
    # Update the doctor's status to approved
    doctor.is_verified = True  # or however you track approval
    doctor.save()
...
def reject_doctor(request, doctor_id):
    ...
    doctor.delete()  # or change status, depending on your requirements
    messages.success(request, 'Doctor registration rejected.')
    return redirect('view_pending_doctors')

def still_pending(request):
    return render(request, 'still_pending.html')
